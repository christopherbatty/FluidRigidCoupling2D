...
drag(int x, int y)
{
   switch(action_mode){
      ...
      case ROTATE:
         ...
         if(pitch<-0.5f*M_PI) pitch=-0.5*M_PI;
         else if(pitch>0.5f*M_PI) pitch=0.5*M_PI;
         break;
      case TRUCK:
...
transform_mouse(int x, int y, float ray_origin[3], float ray_direction[3])
{
   ...
   float scale=0.5f*(float)tan(fovy)/winheight;
...
display(int x, int y)
{
   ...
   if(status==UNINVOLVED){
      ...
      if(status==SELECTED) glColor3d(0.8, 0.8, 0.8);
