...
void FluidSim::process_collisions() {
   ...
   //again, not the ideal way to deal with this.
   vertices.clear();
   rbd->get2DVertices(vertices);
   min_phi = dx;
   min_ind = -1;
   ...
   if (min_phi < 0) {
      Vec2f com0;
      rbd->getCOM(com0);
      rbd->setCOM(com0 - (min_phi)*min_normal);
   }
...
void FluidSim::update_rigid_body_grids()
{
   ...
   //recompute the "effective" masses per axis, so that we can exactly
...
void FluidSim::solve_pressure(float dt) {
   ...
   for (int j = 0; j < nj; ++j) {
      for (int i = 0; i < ni; ++i) {
         ...
         float centre_phi = liquid_phi(i, j);
         if (centre_phi < 0) {
            updated_rigid_linear_velocity[0] += (float)((double)dt*base_trans_x(i, j)*pressure[index] / (double)rigid_u_mass);
            updated_rigid_linear_velocity[1] += (float)((double)dt*base_trans_y(i, j)*pressure[index] / (double)rigid_v_mass);

            updated_rigid_angular_momentum += (float)((double)dt*base_rot_z(i, j) * pressure[index]);
         }
      }
   }
